var express = require('express');
var express = require('express');
var cors = require('cors');
var bodyParser = require('body-parser');
var app = express();

app.use(bodyParser.urlencoded({extended: false }));
app.use(bodyParser.json());
app.use(cors());
app.use(express.static('public'));

app.listen(1311, function(){
    console.log("Server is running ...");
})

const db = require('./admin');
app.get('/', function (req, res) {
    res.send('Hello World');
});

let maxId = -1;
const CRef = db.collection('test1111');

CRef.get().then( (snapshot) => {
    const Items= snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data()
    }))
    console.log(Items);
    let arr = Items;
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].id > maxId) {
            maxId = arr[i].id;
        }
    }
    console.log('maxID: '+maxId);
})

app.get("/get", async(req,res)=>{
    const cRef = db.collection('test1111');
    try {
        cRef.get().then( (snapshot) => {
        const items= snapshot.docs.map((doc) => ({
            id: doc.id,
            ...doc.data()
        }))
        console.log(items);
        res.status(201).json(items);
        })
    } catch(error){
        res.status(500).json({message: error});
    }
    // res.status(201).json(items);
})
    //POST 
app.post('/post', async (req, res) => {
    let maxId = 0;
    const CRef = db.collection('test1111');
    CRef.get().then((snapshot) => {
        const ids = snapshot.docs.map((doc) => (parseInt(doc.id)))
        console.log(ids);
        let arr = ids;
        for (let i = 0; i < arr.length; i++) {
            if (arr[i] > maxId) {
                maxId = arr[i];
            }
        }
        console.log('maxID: ' + maxId);
        let id = maxId+1;
        req.body.id = id;
        const data = req.body;
        const docRef = db.collection('test1111').doc(`${id}`);
        docRef.set(data) .then(() => {
            console.log('Tài liệu đã được tạo thành công!');
        }).catch((error) => {
            console.error('Lỗi khi tạo tài liệu: ', error);
        });
        res.send('add successfully')
        maxId++;
    })
});

//delete
app.delete('/delete/:id', async (req, res) => {
    const id = req.params.id;
    try {
        const documentRef = db.collection('test1111').doc(id);
        documentRef.delete();
        res.send('delete successfully');
    } catch (error) {
        console.error('Error deleting document', error);
    }
});
//update
app.put('/update/:id', async (req, res) => {
    const id = req.params.id;
    const document = db.collection('test1111').doc(id);
    const dataToUpdate = req.body;
    try {
        await document.update(dataToUpdate);
        res.send(`Document ${id} has been updated successfully.`);
    } catch (error) {
        console.error('Error updating document', error);
    }
});